...
// page size should be the same in Job model
$pageSize = 2;
$category = "";
$sort = "CreationDate";
$page = $totalPages = 1;
...
// get all jobs
$jobs = Job::GetAll(0, $category, $sort, $page);
if (count($jobs) > 0) {
  $totalPages =  ceil($jobs[0]->TotalRows / $pageSize);
  if ($page > $totalPages)
    $page = $totalPages;
}
  ...
  <div class="container-fluid">
    <div class="row fill-height">
      <div class="col-3 z-index-2">
        <!-- menu -->
        <?php menu($freelancer->UserTypeId, "jobs", $freelancer->Username); ?>
      </div>
      <!-- page -->
        ...
        <div class="container-fluid">
          ...
          <form class="form" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="GET">
            ...
            <div class="row">
              ...
              <div class="col-2">
                <a href="./freelancer-jobs.php?page=<?php echo $previousPage ?>&category=<?php echo $category ?>&sort=<?php echo $sort ?>" class="btn btn-primary btn-md btn-block <?php if ($previousPage < 1) echo 'disabled'; ?>">Previous</a>
              </div>
              <div class="col-2">
                <a href="./freelancer-jobs.php?page=<?php echo $nextPage ?>&category=<?php echo $category ?>&sort=<?php echo $sort ?>" class="btn btn-primary btn-md btn-block <?php if ($nextPage > $totalPages) echo 'disabled'; ?>">next</a>
              </div>
            </div>
          </form>
        </div>
